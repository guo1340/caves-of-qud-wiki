(function () {
  ...
  function seoFor(route) {
    ...
    if (info.kind === 'category') {
      ...
      description = clip(info.category.summary + ' Caves of Qud archive pages with strategy, FAQ and related links.');
      keywords = ['Caves of Qud ' + info.category.title.toLowerCase(), 'Caves of Qud wiki', 'Caves of Qud guide'];
    } else if (info.kind === 'detail') {
      title = info.page.title + ' | ' + S.titleSuffix;
      description = clip(info.page.summary);
      keywords = ['Caves of Qud ' + info.page.title.toLowerCase(), 'Caves of Qud ' + info.category.title.toLowerCase(), 'Caves of Qud wiki'];
      ogType = 'article';
    } else if (info.kind === 'info') {
      title = info.page.title + ' | ' + S.titleSuffix;
      description = clip(info.page.body);
    } else if (info.kind === 'unknown') {
      title = 'Archive Record Missing | ' + S.titleSuffix;
    }
